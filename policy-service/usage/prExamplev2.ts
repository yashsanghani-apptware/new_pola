...
async function exampleUsage() {
  try {
    ...
    // Create and save a Service Control Policy (SCP)
    const serviceControlPolicy = new PolicyModel({
      apiVersion: 'api.pola.dev/v1',
      name: 'ServiceControlPolicy-LimitActions',
      serviceControlPolicy: {
        version: '2024-08-01',
        boundEntities: {
          users: [user._id as Types.ObjectId], // Apply SCP to the specific user
        },
        maxPermissions: ['read', 'write'], // Restrict actions to only 'read' and 'write'
      },
      auditInfo: { createdBy: 'admin', createdAt: new Date() },
    });
    await serviceControlPolicy.save();
    console.log('Service Control Policy created:', serviceControlPolicy);
